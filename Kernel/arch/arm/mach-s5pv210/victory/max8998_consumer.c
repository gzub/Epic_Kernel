...
static const unsigned int dvs_volt_table_1GHZ[NUM_FREQ][3] = {
	...
	{L4, DVSARM3, DVSINT2},	// 400mhz
	{L5, DVSARM4, DVSINT2},	// 200mhz
...
static int set_max8998(unsigned int pwr, enum perf_level p_lv)
{
	...
	int pmic_val;
	int ret = 0;
	const unsigned int (*frequency_match_tab)[4] =
	    frequency_match[S5PC11X_FREQ_TAB];
...
int set_gpio_dvs(enum perf_level p_lv)
{
	...
	DBG("[PWR] %s : GPH0DAT=%x, GPH0CON=%x, GPH0PUD=%x\n", __func__,
	    readl(S5PV210_GPH0DAT), readl(S5PV210_GPH0CON),
	    readl(S5PV210_GPH0PUD));
...
int set_voltage_dvs(enum perf_level p_lv)
{
	...
	{
		arm_delay =
		    frequency_match_tab[p_lv][1] -
		    frequency_match_tab[step_curr][1];
		int_delay =
		    frequency_match_tab[p_lv][2] -
		    frequency_match_tab[step_curr][2];
	} else			// clk down
	{
		arm_delay =
		    frequency_match_tab[step_curr][1] -
	...
	DBG("[PWR] %s : level (%d -> %d), delay (%u)\n", __func__,
	    frequency_match_tab[step_curr][0], frequency_match_tab[p_lv][0],
	    delay);
	...
	const unsigned int (*dvs_volt_table_ptr)[3] =
	    dvs_volt_table[S5PC11X_FREQ_TAB];
	...
	/*For arm vdd */
	new_voltage =
	    dvs_arm_voltage_set[dvs_volt_table_ptr[p_lv][1] - DVSARM1][1];
	old_voltage =
	    dvs_arm_voltage_set[dvs_volt_table_ptr[step_curr][1] - DVSARM1][1];
	...
	/*For arm int */
	new_voltage =
	    dvs_arm_voltage_set[dvs_volt_table_ptr[p_lv][2] - DVSARM1][1];
	old_voltage =
	    dvs_arm_voltage_set[dvs_volt_table_ptr[step_curr][2] - DVSARM1][1];
...
static int max8998_consumer_probe(struct platform_device *pdev)
{
	int ret = 0;
#ifdef DECREASE_DVFS_DELAY
	unsigned int (*frequency_match_tab)[4] =
	    frequency_match[S5PC11X_FREQ_TAB];
#endif
	...
	if (universal_sdhci2_detect_ext_cd())	//card not present
	{
		...
		for (gpio = S5PV210_GPG2(1); gpio < S5PV210_GPG2(7); gpio++) {
			if (gpio != S5PV210_GPG2(2)) {
				...
				s3c_gpio_slp_setpull_updown(gpio,
							    S3C_GPIO_PULL_NONE);
			}
		}
	}
