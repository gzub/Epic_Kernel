...
static struct usb_string strings_dev[] = {
	...
	[STRING_PRODUCT_IDX].s = "SAMSUNG_Android SPH-D700",
...
recheck:
		...
		/* set product ID to the appropriate value */
		if (enable== USBSTATUS_SAMSUNG_KIES && prev_enable_status != USBSTATUS_ADB)
		{
		...
		else if(enable == USBSTATUS_MTPONLY && prev_enable_status != USBSTATUS_ADB)
		{
...
#if !defined(CONFIG_ARIES_NTT) // disable tethering xmoondash
		else if(enable == USBSTATUS_VTP && prev_enable_status != USBSTATUS_ADB) 
		{
