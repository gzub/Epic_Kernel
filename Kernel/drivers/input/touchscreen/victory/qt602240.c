...
// victory
unsigned char test_node[TEST_POINT_NUM] = {196,       		   188, 		   104,    		       20,     			   12};        
...
void qt_Multitouchscreen_Init(void)
{
    ...
    touchscreen_config.tchthr = 26;	// 40;//45;

    touchscreen_config.tchdi = 2;
    touchscreen_config.orient = 7;// chnaged for victory previous value is 1
...
void qt_Multitouchscreen_stylus_Init(void)
{
    ...
    touchscreen_config.tchthr = 16;	// 25;//45;	
...
//    touchscreen_config.movhystn = 5;///1;
    ...
    if (write_multitouchscreen_config(0, touchscreen_config) != CFG_WRITE_OK)
    {
        QT_printf("[TSP] qt_Multitouchscreen_stylus_Init Configuration Fail!!! , Line %d \n\r", __LINE__);
    }
}
...
void qt_Noise_Suppression_Config_Init(void)
{
	...
	noise_suppression_config.noisethr = 20;	// 30; 	//35;
...
void quantum_touch_probe(void)
{
	...
	for (i = 0; i < NUM_OF_I2C_ADDR; i++){
		if (init_touch_driver( i2c_addresses[i], (void *) &message_handler) == DRIVER_SETUP_OK)
			{
				/* "\nTouch device found\n" */
				printk("\n[TSP] Touch device found\n");
				break;
...
int set_tsp_for_ta_detect(int state)
{
	...
	if(state)
        {
		touchscreen_config.tchthr = 46;	//70;
		noise_suppression_config.noisethr = 13;	//20;		   
        ...
        {
		   touchscreen_config.tchthr = 26;		//40;
		   noise_suppression_config.noisethr = 20;	//30;		   
...
char usb_detect = 0;
char usb_get_message = 0;
int set_tsp_for_usb_detect(int state)
{
	uint16_t object_address;
    	uint8_t *tmp;
    	uint8_t status;
	...
	if(state)
		...
		tmp= &noise_suppression_config.ctrl ;
		status = write_mem(object_address+0, 1, tmp);	
		...
		if ( usb_get_message == 1)
		{
			usb_get_message = 0;
			
			touchscreen_config.blen = 33;
			acquisition_config.chrgtime = 7;
			...
			if (object_address == 0)
			{
			    	printk("\n[TSP][ERROR] GEN_ACQUISITIONCONFIG_T8 object_address : %d\n", __LINE__);
				enable_irq(qt602240->client->irq);
				return -1;
			}
			tmp= &acquisition_config.chrgtime ;
			status = write_mem(object_address+0, 1, tmp);	
...
EXPORT_SYMBOL(set_tsp_for_usb_detect);
	...
	if (driver_setup == DRIVER_SETUP_OK)
	{
	...
	#endif
		{
			...
			//dprintk("[TSP] msg id =  %d ", quantum_msg[0]);
                        ...
                        if(( quantum_msg[0] == 0xF) ||( quantum_msg[1] == 0x09 ))
			{
				if(usb_detect)
				{
					usb_get_message = 1;
...
#ifdef QT_FIRMUP_ENABLE
{
	...
	//ret = request_irq(qt602240->client->irq, qt602240_irq_handler, IRQF_SHARED|IRQF_PROBE_SHARED|IRQF_DISABLED, "qt602240 irq", qt602240);
	//disable_irq(qt602240->client->irq);
	...
	//	enable_irq(qt602240->client->irq);
#if defined(USE_TSP_EARLY_SUSPEND) && defined(CONFIG_HAS_EARLYSUSPEND)
...
static int qt602240_early_suspend(struct early_suspend *h)
{
	...
	if (write_power_config(power_config_sleep) != CFG_WRITE_OK)
	{
        ...
        s3c_gpio_setpin(_3_GPIO_TOUCH_EN, 0);  //suik_Check
	...
	s3c_gpio_cfgpin(GPIO_TOUCH_RST, S3C_GPIO_INPUT);
	s3c_gpio_setpull(GPIO_TOUCH_RST, S3C_GPIO_PULL_DOWN);
...
static int qt602240_late_resume(struct early_suspend *h)
{
	...
	ENTER_FUNC;
        ...
        s3c_gpio_setpin(_3_GPIO_TOUCH_EN, 1);
