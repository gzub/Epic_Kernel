...
static int touchkey_keycode[] = {NULL, TOUCH_KEY_SEARCH, TOUCH_KEY_BACK, TOUCH_KEY_HOME, TOUCH_KEY_MENU};
...
//char set_tsp_for_usb_detect_value  = 0;
//char set_tsp_for_usb_release_value = 0  ;
char set_tsp_for_usb_detect_value  = 0;
char set_tsp_for_usb_release_value = 0  ;

char set_tsp_for_ta_detect_value = 0 ;
char set_tsp_for_ta_release_value = 0  ;
extern void get_touchkey_firmware(char * version);
...
static int i2c_touchkey_write(u8 *val, unsigned int len)
{
	...
	while(retry--)
	{
            ...
            err = i2c_transfer(touchkey_driver->client->adapter, msg, 1);
            if (err >= 0) return 0;
            printk("ANSH:DEBUG %s %d i2c transfer error\n", __func__, __LINE__);
            mdelay(10);
	}
	return err;
}
...
extern void keypad_led_onoff(int OnOff);  //suik_check
...
// the touch_update sys fs file is remove 
#if 0
...
// touch_update sys fs is removed
