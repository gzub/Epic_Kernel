...
max8998_irq_table_type max8998_irq_table[ENDOFIRQ+1] = {
    ...
    NULL, // CHGRSTF
    NULL, // DCINOVPR
    NULL,	// maxim_charging_topoff, // TOPOFFR	// hanapark_Victory
    NULL, // ONKEY1S
...
irqreturn_t pmic_irq(int irq, void *dev_id)
{
	...
	hrtimer_start(&charger_timer,
					ktime_set(800 / 1000, (800 % 1000) * 1000000),
					HRTIMER_MODE_REL);
...
extern char set_tsp_for_usb_detect_value ;
extern char set_tsp_for_usb_release_value ;
extern char set_tsp_for_ta_detect_value ;
extern char set_tsp_for_ta_release_value ;
extern char tsp_initialed ;
void maxim_vac_connect(void)
{
	u8 UsbStatus=0;
	...
	if(Get_MAX8998_PM_REG(VDCINOK_status))
	{
		...
		UsbStatus = FSA9480_Get_USB_Status();
			...
			if( tsp_initialed == 1)
				...
				set_tsp_for_usb_detect_value = 1;
			}
			else{
...
*/
		...
		if(curent_device_type == PM_CHARGER_TA)
		{
			if( tsp_initialed == 1)
				...
				set_tsp_for_ta_release_value = 1;
		}
		else if (curent_device_type == PM_CHARGER_USB_INSERT)
		{
			if( tsp_initialed == 1)		
				...
				set_tsp_for_usb_release_value = 1;
		}
...
void maxim_charging_control(unsigned int dev_type  , unsigned int cmd,int uicharging)//ansari
{
	byte reg_buff[2];
	int value;
