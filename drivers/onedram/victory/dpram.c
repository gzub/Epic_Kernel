...
**
****************************************************************************/
#define CONFIG_DPRAM_CHECK_BOX
#define CONFIG_DPRAM_VERSION_CHECK
//etinum.victory.dpram #define CONFIG_DPRAM_1_1_0
#define CONFIG_DPRAM_1_0_0 //etinum.victory.dpram 
...
#ifdef ENABLE_CP_UPLOAD_PHASE_I 
...
static int onedram_get_semaphore_for_init(const char *func)
{
	...
	for(j = 0; j < req_try; j++) {
		...
		*onedram_mailboxBA = cmd;
//#ifdef PRINT_SEND_IRQ  //suik_Check For checking IRQ Sending & More delay to receive Phone response
...
//	dprintk("START cmd = 0x%x\n", cmd);
	...
	switch (cmd) {
		...
		case DPRAM_PHONE_POWON:
			//dpram_phone_just_power_on();
			printk(KERN_ERR "[OneDRAM] - DPRAM_PHONE_POWON ioctl.\n");
      		        phone_sync = 0;
			dump_on = 0;
			requested_semaphore = 0;
			unreceived_semaphore = 0;
		...
		case DPRAM_PHONEIMG_LOAD:
            printk("[DPRAM] DPRAM_PHONEIMG_LOAD called!!!! \n");
			//etinum.victory.dpram dpram_phone_image_load();
			return 0;
		...
		case DPRAM_PHONE_RESET:
			phone_sync = 0;     //suik_Fix Adding ATLAS
			requested_semaphore = 0;
			unreceived_semaphore = 0;				
			dpram_phone_reset();
			return 0;
...
#if 0  // old DPRAM did not have below define.
	/* @LDK@ raw check. */
...
//	printk("[OneDRAM IRQ handler]: mailboxAB 0x%8x\n", *onedram_mailboxAB);
	//printk("=====> %s  mailboxAB 0x%8x\n", __func__, *onedram_mailboxAB);

	if(mailboxAB == 0x12341234) {
		printk("  - CP -> AP (mailboxAB): 0x12341234\n");
		printk("  - PSI download OK.\n");
		return IRQ_HANDLED;
	}else if(mailboxAB == 0xabcdabcd) {
		phone_boot = 1;
		printk("\n  - CP -> AP (mailboxAB): 0xabcdabcd\n"); 
		return IRQ_HANDLED;
	}
	check_int_pin_level();
...
static void init_hw_setting(void)
{
	...
	if (gpio_is_valid(GPIO_PHONE_RST_N)) {
		if (gpio_request(GPIO_PHONE_RST_N, "GPH3"))
