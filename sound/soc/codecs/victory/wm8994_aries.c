...
// Ringtone
// Speaker
#define TUNING_RING_SPKL_VOL		0x3F		// 26h 1018 ringtone +1dB
#define TUNING_RING_CLASSD_VOL		0x6		// 25h 1018 ringtone +1.5dB
...
//Dual
//#define TUNING_DUAL_DAC1L_RADIO_VOL		0x70		// 402h  FROYO_MERGE
//#define TUNING_DUAL_DAC1R_RADIO_VOL		0x70		// 403h  FROYO MERGE
...
{
	wm8994_write( codec,WM8994_AIF1_CONTROL_2, 0x8000);
/* Volume Setting */
...
{
	struct wm8994_priv *wm8994 = codec->private_data;
        unsigned int headset_type = get_headset_status();
	int val;
	...
	else if(headset_type == SEC_HEADSET_3_POLE_DEVICE)
    {
        wm8994_write(codec, 0x0028, 0x0030);    // Input Mixer 2
        wm8994_write(codec, 0x0002, 0x6240);    // Power Management 2
        wm8994_write(codec, 0x0029, 0x0030);    // Input Mixer 
	...
	gpio_set_value(S5PV210_GPB(1), 1);
	audio_ctrl_mic_bias_gpio(1);
...
void wm8994_set_end_point_volume(struct snd_soc_codec *codec, int path)
{
    ...
    switch(path)
    {
        case RCV :
        {
            wm8994_set_voicecall_receiver(codec);
//            wm8994_write(codec, 0x001F, (TUNING_HPOUT2_VOL << 0x0004));    // HPOUT2 Volume
        ...
        case HP :
        {
            wm8994_set_voicecall_headset(codec);
//            if(tty_mode == TTY_MODE_HCO)
//            {
...
//            wm8994_write(codec, 0x0026, (0x01C0 | TUNING_CALL_SPKL_VOL));    // Speaker Volume Left
        ...
        case BT :
        {
            wm8994_set_voicecall_bluetooth(codec);
